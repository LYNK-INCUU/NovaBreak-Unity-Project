    ...
    public class PlayerMovement : MonoBehaviour
    {
       ...
       "       if false: Speed while shooting = walkSpeed")]
        ...
        public void Look()
        {
            ...
            float mouseX = (InputManager.mousex * InputManager.sensitivity_x * Time.fixedDeltaTime + InputManager.controllerx * Time.deltaTime * InputManager.controllerSensitivityX) * inverted * sensM;
            float mouseY = (InputManager.mousey * InputManager.sensitivity_y * Time.fixedDeltaTime * inverted + InputManager.controllery * Time.deltaTime * InputManager.controllerSensitivityY * -inverted) * sensM;
            ...
            if (wallRunning && canWallRun) cameraTilt = wallLeft ? Mathf.Lerp(cameraTilt, -wallrunCameraTiltAmount, Time.deltaTime * cameraTiltTransitionSpeed) : Mathf.Lerp(cameraTilt, wallrunCameraTiltAmount, Time.deltaTime * cameraTiltTransitionSpeed);
            else if (isCrouching && rb.linearVelocity.magnitude >= walkSpeed && allowSliding && !hasJumped) cameraTilt = Mathf.Lerp(cameraTilt, slidingCameraTiltAmount, Time.deltaTime * cameraTiltTransitionSpeed);
            else cameraTilt = Mathf.Lerp(cameraTilt, 0, Time.deltaTime * cameraTiltTransitionSpeed);
        ...
        private void CheckGrounded()
        {
            ...
            {
                Vector3 origin = center + offset + Vector3.up * 0.1f;

                if (Physics.SphereCast(origin, radius * 0.5f, Vector3.down, out RaycastHit hit, castDistance, whatIsGround))
                {
