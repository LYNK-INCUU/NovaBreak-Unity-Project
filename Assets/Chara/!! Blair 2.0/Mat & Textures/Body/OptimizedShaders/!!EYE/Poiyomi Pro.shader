Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/fb234be6712fb59428a19143744695b8"
{
	Properties
	{
		...
		[HideInInspector] s_start_MainHueShift_2121EYE ("Hue Shift--{reference_property:_MainHueShiftToggle,persistent_expand:true,default_expand:true}", Float) = 1
		...
		_MainHueShift_2121EYE ("Hue Shift", Range(0, 1)) = 0
		...
		[HideInInspector] s_end_MainHueShift_2121EYE ("Name Motion", Float) = 0
		...
		_EmissionHueShift_2121EYE ("Hue Shift", Range(0, 1)) = 0
		...
		_GlitterHueShift_2121EYE ("Hue Shift", Range(0, 1)) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _EmissionHueShift_2121EYE;
			...
			float _GlitterHueShift_2121EYE;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = hueShift(emissionColor0, frac(_EmissionHueShift_2121EYE + (0.0 /*_EmissionHueShiftSpeed*/) * _Time.x) * (1.0 /*_EmissionHueShiftEnabled*/), (0.0 /*_EmissionHueShiftColorSpace*/));
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[(0.0 /*_GlitterUV*/)] + float4(0.1,0.1,0,0).xy * _Time.x) * (300.0 /*_GlitterFrequency*/);
					...
					switch((0.0 /*_GlitterShape*/))
					{
						...
						if ((0.0 /*_GlitterRandomRotation*/) == 1 || (0.5 /*_GlitterTextureRotation*/) != 0 || float4(0,0,0,0).x != 0 || float4(0,0,0,0).y != 0 || glitterRotationTimeOffset != 0)
						{
							...
							if (glitterRandomRotationSpeed.x + glitterRandomRotationSpeed.y + (0.5 /*_GlitterTextureRotation*/) == 0 && glitterRotationTimeOffset != 0)
							{
								glitterRandomRotationSpeed = 1;
							}
							float theta = radians((randomBoy + (_Time.x + glitterRotationTimeOffset) * ((0.5 /*_GlitterTextureRotation*/) + glitterRandomRotationSpeed)) * 360);
							float cs = cos(theta);
							float sn = sin(theta);
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, (37.2 /*_GlitterAngleRange*/), (0.3 /*_GlitterSpeed*/), glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, (37.2 /*_GlitterAngleRange*/));
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.0 /*_GlitterBias*/)), poiCam.viewDir)), (13.0 /*_GlitterContrast*/)), 0);
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (13.0 /*_GlitterContrast*/)), 0);
					...
					if ((0.0 /*_GlitterRandomRotation*/) == 1 || (0.5 /*_GlitterTextureRotation*/) != 0 || float4(0,0,0,0).x != 0 || float4(0,0,0,0).y && !(0.0 /*_GlitterShape*/) || glitterRotationTimeOffset != 0)
					{
					...
					{
						glitterColor.rgb = hueShift(glitterColor.rgb, _GlitterHueShift_2121EYE + _Time.x * (0.0 /*_GlitterHueShiftSpeed*/), (0.0 /*_GlitteHueShiftColorSpace*/));
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					float shift = _MainHueShift_2121EYE;
					#ifdef POI_AUDIOLINK
